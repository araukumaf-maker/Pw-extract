...
const entityKey = (entity: { id?: string | null; slug: string }) => entity.id ?? entity.slug;
...
function StudyDashboard({ onLogout, onSessionExpired }: { onLogout: () => void; onSessionExpired: () => void }) {
  ...
  const activeBatch = useMemo(() => batches.find((batch) => entityKey(batch) === selectedBatchSlug) ?? batches[0], [batches, selectedBatchSlug]);
  const activeBatchKey = activeBatch ? entityKey(activeBatch) : '';
  const subjectsQuery = useListPwSubjects(activeBatchKey, { query: { queryKey: getListPwSubjectsQueryKey(activeBatchKey), enabled: Boolean(activeBatchKey), staleTime: 60_000 } });
  const subjects = subjectsQuery.data ?? [];
  const activeSubject = useMemo(() => subjects.find((subject) => entityKey(subject) === selectedSubjectSlug) ?? subjects[0], [subjects, selectedSubjectSlug]);
  const activeSubjectKey = activeSubject ? entityKey(activeSubject) : '';
  const topicsQuery = useListPwTopics(activeBatchKey, activeSubjectKey, { query: { queryKey: getListPwTopicsQueryKey(activeBatchKey, activeSubjectKey), enabled: Boolean(activeBatchKey && activeSubjectKey), staleTime: 60_000 } });
  const topics = topicsQuery.data ?? [];
  const activeTopic = useMemo(() => topics.find((topic) => entityKey(topic) === selectedTopicSlug) ?? topics[0], [topics, selectedTopicSlug]);
  const activeTopicKey = activeTopic ? entityKey(activeTopic) : '';
  const notesQuery = useListPwNotes(activeBatchKey, activeSubjectKey, activeTopicKey, { query: { queryKey: getListPwNotesQueryKey(activeBatchKey, activeSubjectKey, activeTopicKey), enabled: Boolean(activeBatchKey && activeSubjectKey && activeTopicKey) } });
  const dppQuery = useListPwDpp(activeBatchKey, activeSubjectKey, activeTopicKey, { query: { queryKey: getListPwDppQueryKey(activeBatchKey, activeSubjectKey, activeTopicKey), enabled: Boolean(activeBatchKey && activeSubjectKey && activeTopicKey) } });
  ...
  useEffect(() => {
    if (batches.length && !batches.some((batch) => entityKey(batch) === selectedBatchSlug)) setSelectedBatchSlug(entityKey(batches[0]));
  }, [batches, selectedBatchSlug]);
  useEffect(() => {
    if (subjects.length && !subjects.some((subject) => entityKey(subject) === selectedSubjectSlug)) setSelectedSubjectSlug(entityKey(subjects[0]));
  }, [subjects, selectedSubjectSlug]);
  useEffect(() => {
    if (topics.length && !topics.some((topic) => entityKey(topic) === selectedTopicSlug)) setSelectedTopicSlug(entityKey(topics[0]));
